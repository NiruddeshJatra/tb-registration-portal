...
- **`collects_strava_link` does NOT put a field on the public form.** A virtual run's Strava link cannot exist at registration time — the run happens later. The flag renders an instruction telling the athlete to WhatsApp the link after their run; the value reaches the DB through the admin panel (`ManualAddPage` / the detail drawer), and `register_participant()` always sends `p_strava_link: null`. `events.participation_note` carries the deadline/WhatsApp/medal copy shown as HOW IT WORKS on the rail — keep event-specific dates there, not in components.
- **Migrations `0003`–`0010` are all applied to the live DB.** `0004` (Rocket/Upay) + `0005` (txid 8–15, venue/fee/methods refresh, `public_event_slots`, dup-phone guard), `0006` (bike type + Strava columns, both RPCs re-signed, duathlon moved to 6 Nov 2026, Chatto Metro Virtual Run seeded), `0007` (`is_virtual` + the virtual run's t-shirt chart), `0008` (its fee note) `0009` (category-id + bike-type guards, from code review) and `0010` (`participation_note`) were applied via the Supabase MCP against project `tb-registration-portal`. `0006` went in as four numbered steps (`0006a`–`0006d`) because each MCP call is its own transaction. Future migrations follow the same "written here, applied manually" convention — don't assume a new file is live until confirmed.
